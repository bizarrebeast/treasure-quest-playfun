export class FreeLife {
  ...
  constructor(scene: Phaser.Scene, x: number, y: number) {
    ...
    // Create container for heart crystal sprite
    this.sprite = scene.add.container(x, adjustedY)
    
    // Create heart crystal sprite from URL
    this.heartSprite = scene.add.image(0, 0, 'heart-crystal')
    
    // Set size same as diamond (29x29)
    this.heartSprite.setDisplaySize(29, 29)
    
    this.sprite.add(this.heartSprite)
    ...
    // Add pulsing animation to container (same as diamond)
    scene.tweens.add({
      targets: this.sprite,
      scaleX: 1.1,
      scaleY: 1.1,
    ...
    // Add sparkle effect (same as diamond)
  ...
  private createSparkle(): void {
    ...
    // Create star-shaped sparkle with pink/red color for hearts
    sparkle.fillStyle(0xff69b4, 0.9) // Hot pink color
    sparkle.beginPath()
    for (let i = 0; i < 8; i++) {
  ...
  collect(): void {
    ...
    // Heart collection animation with brilliant flash (same as diamond)
    this.scene.tweens.add({
      targets: this.sprite,
      scaleX: 2.5,
      scaleY: 2.5,
